...
export const faceVerificationApiProvider = (app) => {
  const businessServices = new BusinessServicesController();

  app.post(
    "/api/v1/face_verfication",
    authenticateUser,
    async (req, res, next) => {
      try {
        const user = req.user;

        const userDetails = await businessServices.fineFaceVerification(user);

        if (userDetails.length > 0) {
          return res.status(StatusCodes.UNAUTHORIZED).json({
            message: "Face verification already done by this user",
          });
        }

        const business = await businessServices.CreateFaceVerification({
          ...req.body,
